...
*/ 
	...
	*/
		...
		Tipsy.prototype = {
			...
			tip: function() {
				if (!this.$tip) {
					this.$tip = $('<div className="tipsy"></div>').html('<div className="tipsy-arrow"></div><div className="tipsy-inner"/></div>');
				}
				return this.$tip;
			},
	...
	*/
	...
	$.fn.player = function(settings) {
		...
		this.each(function() {
			$(this).wrap('<div className="simple-player-container" />').parent().prepend(
				'<div><ul>' + 
					'<li style="display: inline-block; padding: 0 2px; "><a style="text-decoration: none;"' +
					' className="start-button" href="javascript:void(0)">' + playControl + '</a></li>' + 
				'</ul></div>'
			);
